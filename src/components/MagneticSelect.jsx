...
/* Magnetic select — from Bencho (bencho.dev), MIT licensed:
   bencho.dev/licence. Ported to JSX for this project; every
   change from the original is marked "JustMyType:". */

/* MARKS was Bencho's own pictures, which are not licensed
   to travel. Point this at yours. */
/* JustMyType: left as the stub. The mood picker passes its own
   pictures through the `options` prop (src/data/moodMarks.js),
   so this only matters if the component is used bare. */
const MARKS = [];
...
/* JustMyType: nothing in this block calls useSpring — the chips
   move on Framer's springs — but it is kept, notes and all, as
   it shipped. */
...
function useSpring(target, tune = 50, instant = false) {
  ...
  useEffect(() => {
    if (instant) {
      cur.current = target;
      vel.current = 0;
      // eslint-disable-next-line react-hooks/set-state-in-effect -- JustMyType: kept as shipped; unused here
      setAt(target);
      return;
    }
...
export default function MagneticSelect({
  ...
  give = 50,
  /* JustMyType: the component was a closed demo — its own
     pictures and its own state. These four make it a real
     control: `options` is [{ id, label, mark }] in cluster
     order (the first sits at the hub), `value`/`onChange` make
     it controlled, and `label` names the radiogroup. All
     optional; without them it behaves as it shipped. */
  options,
  value,
  onChange,
  label = "Shape",
} = {}) {
  const marks = options ? options.map((o) => o.mark) : MARKS;
  const names = options ? options.map((o) => o.label) : NAMES;
  const n = Math.min(SIZES[size] ?? 7, marks.length);
  const p = clamp(pull, 0, 100) / 100;
  /* JustMyType: the table only holds 3 and 7, so any other
     count came back undefined and threw. The comment above
     says the points are the lattice taken in rings outward,
     so a count up to seven is the first n of the hexagon —
     six is the "centre with a ring one short of closing" it
     describes, which is exactly the six moods. */
  const pts = CLUSTER[n] ?? CLUSTER[7].slice(0, n);
  ...
  const [ownSel, setSel] = useState(0);
  /* JustMyType: controlled when `value` is given */
  const controlled = options && value !== undefined;
  const sel = controlled ? Math.max(0, options.findIndex((o) => o.id === value)) : ownSel;
  ...
  /* JustMyType: written after render rather than during it —
     React's rules keep ref writes out of the render pass. The
     pointer handler only reads it on a later event, so it sees
     the same numbers either way. */
  useLayoutEffect(() => { hub.current = hubNow; });
  ...
  useEffect(() => {
    ...
    let raf = 0;
    let next = { x: 0, y: 0 };
    /* JustMyType: the listener is on the document, so it hears
       every pointer move on the page — and here the block lives
       in a panel that stays mounted while collapsed. Publishing
       only when the lean actually changes keeps a still (0, 0)
       from re-rendering the cluster on every move elsewhere. */
    let shown = { x: 0, y: 0 };
    const publish = () => {
      raf = 0;
  ...
  const choose = (i) => {
    if (i === at) return;
    setSel(i);
    /* JustMyType: report the choice by the option's id */
    if (options) onChange?.(options[i].id);
  };

  return (
    <div className="mag" ref={wrap} style={{ width: W, height: H }} role="radiogroup" aria-label={label}>
      {pts.map(([px, py], i) => {
        const on = i === at;
